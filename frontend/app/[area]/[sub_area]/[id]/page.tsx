...
export default async function ShopDetailPage({ params }: { params: Promise<Params> }) {
  ...
  const { name: shopName, area: areaName, sub_area: subAreaName, latitude, longitude, address } = shopData;

  return (
    <BreadcrumbProvider value={{ area, areaName, subArea: sub_area, subAreaName, shopName }}>
      <Breadcrumb />
